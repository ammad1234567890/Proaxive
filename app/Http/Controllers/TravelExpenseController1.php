...
class TravelExpenseController extends Controller {
    ...
    public function add_travelexpense() {
        ...
        $data['users'] = User::select('name', 'id')->whereIn('role_id', [1, 2, 4])->get();
            ...
            try {
                ...
                if ($request->expense_type_list == "travel expense") {
                    $data = TravelExpense::create([
                                ...
                                'travelexpense_no' => $request->travel_expense_id,
                                ...
                                'payment_order_number' => e($request->payment_order_no),
                ...
                if ($request->expense_type_list == "general expense") {
                    $data = TravelExpense::create([
                                ...
                                'payment_order_number' => $request->payment_order_no,
                                'email_date' => $request->email_date,
                                'currency_id' => $request->expense_currency,
                                'by_order_of' => $request->by_order_of,
                ...
                for ($i = 0; $i < sizeof($request->expense_types); $i++) {
                    ...
                    $sort[$i]['date'] = $request->expense_date[$i];
                ...
                $data_email = ['employee' => $request->employee
                    , 'purpose' => $request->travel_purpose
                    , 'amount' => $request->total_amount
                    , 'redirection_url' => URL::to('expense/details/view/' . $data->id)
                ];
    ...
    public function expense_details_data($id) {
        return DB::table('travel_expense')
                        ...
                        ->select('travel_expense.*'
                                ...
                                , 'loc_on_account_of.name as location_account_of_name', 'travel_status.name as status'
                        )
...
//expense list grid for  md end
    ...
    public function expense_status_check($expense_id) {
        return DB::table('expense_approvals')
                        ->join('users', 'users.id', '=', 'expense_approvals.user_id')
                        ->where(['expense_id' => $expense_id, 'role_id' => Auth::user()->role_id])->count();
    }
...
// status hrapprovel check for md end
    public function expense_status_update(Request $request) {
        if (isset($request->status)) {
            if ($request->status != "") {
                ...
                return redirect()->back()->with('error_message', 'Expense Status Updated');
            } else {
                return ['status' => false];
            }
        } else {
    ...
    public function travel_expense_details($expense_id) {
        $employee = DB::table('travel_expense')
                ...
                ->select('travel_expense.employee_id', 'travel_expense.travel_purpose'
                        , 'travel_expense.total_amount', 'users.name')
                ->first();
        $data_email = ['employee' => $employee->name
    ...
    public function travel_expense_details_for_md($expense_id) {
        ...
        $this->sent_email_to_customer($data_email);
        return true;
    }

    public function sent_email_to_customer($data) {
        $hr_email = [];
        $email_list = User::where(['role_id' => 3])->select('email')->get();
        for ($i = 0; $i < sizeof($email_list); $i++) {
            array_push($hr_email, $email_list[$i]->email);
        }
        Mail::send(['html' => 'approval_customer_email'], $data, function($message) use($hr_email) {
