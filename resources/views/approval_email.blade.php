...
<table border="0" cellpadding="0" cellspacing="0" align="center"  width="580" style=" border:thin solid #eee;">
 ...
 <td>
   <table  height="62" border="0"  bgcolor="#f6fbff"  cellpadding="0" cellspacing="0" align="left"  width="360px" style="color:#0d6e8c; font-family:Calibri; font-size:12px;  margin-left:5px; border:#CCCCCC dotted thin; ">
    ...
    <table style="border: 1px solid gray;" width="100%" height="62" border="0"  bgcolor="#f6fbff"  cellpadding="0" cellspacing="0" align="left">
            @foreach($approval as $key => $data)
            <tr>
                @if($data->user->role_id==2)
                <td style="width:30%;" style="color:#222;">{{$data->user->name}}(HR)</td>
                @elseif($data->user->role_id==1)
                <td style="width:30%;" style="color:#222;">{{$data->user->name}}(MD)</td>
                @endif
                <td style="width:70%;" style="color:#222; text-align:center ">: <?php if($data->comment!=''){ echo $data->comment; }else{echo '----';} ?></td>
            </tr>
            @endforeach
    </table>
